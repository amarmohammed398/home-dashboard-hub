...
- **Electricity / gas usage** — via a UK smart meter data source
  (n3rgy's free consumer API, or a Hildebrand Glow CAD). Genuinely
  blocked right now on a missing physical device, not on anything
  about the plan itself: this household's IHD was never issued
  (Scottish Power stock shortage at installation), an old IHD from a
  previous address can't be re-paired to a different meter
  (IHD-to-meter pairing is a one-time, non-consumer-reversible step),
  and Hildebrand's own CAD device was out of stock with contradictory
  signals on their own site about restock timing. The meter itself is
  confirmed fine — a Honeywell AS302P, confirmed SMETS2 directly from
  its own label — and prepayment status was confirmed not to be a
  blocker either. Gas rides the exact same n3rgy enrollment as
  electricity once unblocked (a second identifier, MPRN, alongside
  MPAN) — not a separate infrastructure problem. Expect gas readings to
  be sparser/more delayed than electricity's own once it's live — UK
  smart gas meters run on battery, not mains power, so their comms
  module transmits less often. **A version of this display was actually
  built and shipped on 30 Aug 2026** (a single grid carbon-intensity
  card, from the UK's free Carbon Intensity API, as a placeholder for
  the still-blocked personal-consumption side) **and was reverted the
  next day at the user's explicit request** — not because the data was
  wrong, but because that specific interim approach wasn't wanted; see
  CHANGELOG.md's two dated entries for the full build-then-revert
  story. When this gets revisited, lead with genuinely useful personal
  information (usage, cost, trends) once meter access exists, rather
  than defaulting back to grid carbon intensity as an opening card
  without checking first — that specific framing is what didn't land.
  If a dynamic tariff ever becomes relevant, a "current rate + cheapest
  window tonight" view would be genuinely useful too.
